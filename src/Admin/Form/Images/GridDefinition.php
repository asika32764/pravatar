	...
	protected function doDefine(Form $form)
	{
		...
		$this->group('search', function (Form $form)
		{
			...
			$this->list('field')
				...
				->option(Translator::translate('admin.image.field.title'), 'image.title')
				->option(Translator::translate('admin.image.field.alias'), 'image.alias');
		 ...
		 *
		 * You can override filter actions in ImagesModel::configureFilters()
		 */
		$this->group('filter', function(Form $form)
		{
			// State
			$this->list('image.state')
				->label('State')
				// Add empty option to support single deselect button
				->option('', '')
				->option(Translator::translate('admin.image.filter.state.select'), '')
